...
class PikInformParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;

    const ROOT_SRC = "https://pikinform.ru";
    ...
    private static function inflatePostContent(NewsPost $post, Curl $curl)
    {
        ...
        $picHolder = $crawler->filter("main.content div[itemprop=\"image\"] img");
        if($picHolder->count() !== 0){
            $post->image = self::normalizeUrl($picHolder->attr("src"));
        }


        $body = $crawler->filter("article.entry div.entry-the-content");
        ...
        foreach ($body->children() as $bodyNode) {
            ...
            if ($node->matches("div, figure") && $node->filter("img")->count() !== 0) {
                ...
                if($post->image === null){
                    ...
                    if($node->filter("figcaption")->count() !==0 ){
                        self::addText($post, $node->filter("figcaption")->text());
                    }
                }
            }
            ...
            if ($node->matches("blockquote") && !empty(trim($node->text(), "\xC2\xA0"))) {
                self::addQuote($post, $node->filter("p")->text());
                continue;
            }
