...
class NashaGazetaParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;

    const ROOT_SRC = "https://nashagazeta.ch";
    ...
    private static function inflatePostContent(NewsPost $post, Curl $curl)
    {
        ...
        $picHolder = $crawler->filter("div.content div.field-name-field-image img");
        if ($picHolder->count() !== 0) {
            $post->image = self::normalizeUrl($picHolder->attr("src"));
        }

        $descHolder = $crawler->filter("div.content div.pane-content");
        if ($descHolder->count() !== 0 && !empty(trim($descHolder->text(), "\xC2\xA0"))) {
            $post->description = Helper::prepareString($descHolder->text());
        }
        $body = $crawler->filter("div.content div.statia div.field-item");

        if ($body->count() === 0) {
            $body = $crawler->filter("div.content div.pane-content");
        }
        if ($body->count() === 0) {
