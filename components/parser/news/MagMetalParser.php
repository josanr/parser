...
class MagMetalParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;

    const ROOT_SRC = "http://magmetall.ru";

    const FEED_SRC = "/xml/rss.xml";
    const LIMIT = 100;
    ...
    private static function inflatePostContent(NewsPost $post, Curl $curl)
    {
        ...
        $picHolder = $crawler->filter("body > div:nth-child(1) > table > tr:nth-child(3) > td:nth-child(2) > table > tr:nth-child(4) > td > img");
        if ($picHolder->count() !== 0) {
            $post->image = self::normalizeUrl(self::ROOT_SRC . $picHolder->attr("src"));
        }
        $body = $crawler->filter("body > div:nth-child(1) > table > tr:nth-child(3) > td:nth-child(2) > table > tr:nth-child(4) > td > div");
