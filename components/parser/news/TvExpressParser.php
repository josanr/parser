...
class TvExpressParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;

    const ROOT_SRC = "http://tv-express.ru";

    const FEED_SRC = "/lenta-novostej";
    const LIMIT = 100;
    ...
    public static function run(): array
    {
        ...
        for ($pageId = 1; $pageId <= ceil(self::LIMIT / self::NEWS_PER_PAGE); $pageId++) {
            ...
            $listSourcePath = self::ROOT_SRC . self::FEED_SRC . "/apage-" . $pageId;
            ...
            $items = $crawler->filter("div.eventrubfirst, div.eventrub");
            if ($items->count() === 0) {
    ...
    private static function inflatePost(Crawler $postData): NewsPost
    {
        ...
        $original = self::normalizeUrl(self::ROOT_SRC . $postData->filter("h3 a")->attr("href"));

        $dateString = $postData->filter("span")->text();
        ...
        $dateString = trim($dateArr[2], ",") . "-" . self::MONTHS[$dateArr[1]] . "-" . $dateArr[0] . " " . $dateArr[3] . "+03:00";
