...
class AppTodayParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;

    const ROOT_SRC = "https://apptoday.ru";

    const FEED_SRC = "/publication/novosti";
    const LIMIT = 100;
    ...
    public static function run(): array
    {
        ...
        $items = $crawler->filter("div.first_teaser, div.main_teaser");
        if ($items->count() === 0) {
    ...
    {
        $title = $postData->filter("div.main_teaser_text a")->text();

        $original = self::ROOT_SRC . $postData->filter("div.main_teaser_text a")->attr("href");
        $createDateString = $postData->filter("div.show-date")->text();
        $createDate = DateTime::createFromFormat("d.m.Y", $createDateString);
        if ($createDate === false) {
    ...
    private static function inflatePostContent(NewsPost $post, Curl $curl)
    {
        ...
        $body = $crawler->filter("div.detail_pub_container");
        ...
        foreach ($body->children() as $bodyNode) {
            ...
            if ($node->matches("div.detail_tags, div.views, div#mpRWidget, div#marketWidget1")) {
                continue;
            }
