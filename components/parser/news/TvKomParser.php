...
class TvKomParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;

    const ROOT_SRC = "https://tvcom-tv.ru";

    const FEED_SRC = "/news-list.html";
    const LIMIT = 100;
    ...
    public static function run(): array
    {
        ...
        $now = new DateTime();
        $counter = 0;
        for ($pageId = 1; $pageId <= 30; $pageId++) {

            $listSourcePath = self::ROOT_SRC . self::FEED_SRC . "?date=" . $now->format("Y-m-d");
            $now->modify("-1 day");
    ...
    {
        $title = $postData->filter("h6")->text();

        $original = self::ROOT_SRC . "/" . self::normalizeUrl($postData->filter("a")->attr("href"));

        $dateString = $postData->filter("div.image-news--time p.todate")->text();
        $dateArr = explode(" ", $dateString);
        if (count($dateArr) !== 3) {
            throw new Exception("Date format error");
        }
        if (!isset($dateArr[1]) || !isset(self::MONTHS[mb_strtolower($dateArr[1])])) {
            throw new Exception("Could not parse date string");
        }

        $dateString = $dateArr[2] . "-" . self::MONTHS[mb_strtolower($dateArr[1])] . "-" . $dateArr[0] . " " . date("H:i:s") . "+08:00";
    ...
    private static function inflatePostContent(NewsPost $post, Curl $curl)
    {
        ...
        $videoContainer = $crawler->filter("main div.container > div iframe");
        if ($videoContainer->count() !== 0) {
            self::addVideo($post, $videoContainer->attr("src"));
        }


        $body = $crawler->filter("main div.container div.video-review div.main-content div.text-block");
