...
class ScrollNewsParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;

    const ROOT_SRC = "https://scroll.news";
    ...
    private static function inflatePostContent(NewsPost $post, Curl $curl)
    {
        ...
        foreach ($body->children() as $bodyNode) {
            ...
            if ($node->matches("p") && $node->filter("img")->count() !== 0) {
                ...
                if(empty($image->attr("data-src"))){
                    $src = self::normalizeUrl(self::ROOT_SRC . $image->attr("src"));
                }else{
                    $src = self::normalizeUrl(self::ROOT_SRC . $image->attr("data-src"));
                }
            ...
            if ($node->matches("p") && !empty(trim($node->text(), "\xC2\xA0"))) {
                if(mb_stripos($node->text(), "Читать ещё •••") !== false){
                    break;
                }
            ...
            if ($node->matches("ul") && $node->filter("img")->count() !== 0) {
