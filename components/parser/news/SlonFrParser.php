...
class SlonFrParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;

    const ROOT_SRC = "https://slon.fr";
    ...
    private static function inflatePostContent(NewsPost $post, Curl $curl)
    {
        ...
        $picHolder = $crawler->filter("div.btArticleContent div.bpbItem img");
        if ($picHolder->count() !== 0) {
            $post->image = self::normalizeUrl($picHolder->attr("src"));
        }

        $body = $crawler->filter("div.btArticleContent div.btArticleBody div.bt_bb_wrapper");
