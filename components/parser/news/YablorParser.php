...
class YablorParser implements ParserInterface
{
    const USER_ID = 2;
    const FEED_ID = 2;

    const ROOT_SRC = "https://yablor.ru";
    ...
    private static function inflatePostContent(NewsPost $post, Curl $curl)
    {
        ...
        $body = $crawler->filter("div#id_well");
        ...
        foreach ($body->getNode(0)->childNodes as $bodyNode) {
            ...
            if ($node->matches("span.title_info")) {
                continue;
            }
            ...
            if ($node->matches("div, img") && $node->filter("img")->count() !== 0) {
